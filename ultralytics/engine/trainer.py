...
class BaseTrainer:
    ...
    def _setup_train(self, world_size):
        ...
        if self.args.teacher_weight:
            self.teacher_model = self.args.teacher_weight
            teacher_ckpt = self.setup_teacher_model()
            self.teacher_model = self.teacher_model.to(self.device)
            self.set_teacher_model_attributes()
            LOGGER.info(f'Load teacher model from {self.args.teacher_weight}')  # report
        ...
        if world_size > 1:
            ...
            if self.args.teacher_weight:
                self.teacher_model = DDP(self.teacher_model, device_ids=[RANK])
