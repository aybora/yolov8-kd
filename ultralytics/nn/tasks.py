...
from ultralytics.utils.anchor import make_center_anchors
from ultralytics.utils.mask import center_to_corner, find_jaccard_overlap, corner_to_center
...
class BaseModel(nn.Module):
    ...
    def forward(self, x, teacher=None, student=None, mask=None, *args, **kwargs):
        """
        ...
        if isinstance(x, dict):  # for cases of training and validating while training.
            if teacher != None:
                return self.loss(x, teacher=teacher, student=student, mask=mask, *args, **kwargs)
            return self.loss(x, *args, **kwargs)
        return self.predict(x, target=None, fw=True, *args, **kwargs)

    def predict(self, x, profile=False, visualize=False, augment=False, target=None, fw=False):
        """
        ...
        if target != None:
            ...
            preds, features = self._predict_once(x, profile=profile, visualize=visualize, target=target)
            mask = self._get_imitation_mask(features, target).unsqueeze(1)
            return preds, features, mask
        return self._predict_once(x, profile, visualize)  # single-scale inference, train

    def _predict_once(self, x, profile=False, visualize=False, target=None):
        """
        Perform a forward pass through the network.
    ...
    def loss(self, batch, preds=None, teacher=None, student=None, mask=None):
        """
        Compute loss
        ...
        preds = self.forward(batch['img'], teacher=teacher, student=student, mask=mask) if preds is None else preds
        return self.criterion(preds, batch, teacher=teacher, student=student, mask=mask)
